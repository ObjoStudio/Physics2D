...
| 10. API completion and debug rendering | **Complete** (2026-09-02) | Inventory reconciliation (2026-09-01): scripted every `Class.Member` mapping in the PORTING inventory against the generated dist and resolved all mismatches — renamed mappings to the implemented idiomatic surface (`World.ContinuousEnabled`/`SpeculativeEnabled`/`WarmStartingEnabled` settable properties, `World.EnableSleeping` read, `World.Events` batch view, `World.ContactHertz`/`ContactDampingRatio`/`MaxContactPushSpeed` as the contact-tuning surface, `Joint.GetJointType`, `Joint.GetConstraintHertz`/`GetConstraintDampingRatio`, `DynamicTree` constructor and `ProxyCount` property), implemented the genuinely missing members (`World.RebuildStaticTree` + `BroadPhase.RebuildStaticTree` full static-tree quality rebuild, and the body-level event enablers `Body.EnableContactEvents`/`EnableHitEvents` writing through to every attached shape with the documented `AreContactEventsEnabled`/`AreHitEventsEnabled` getters — upstream has only the setters), verified the sleeping-body semantics of moving static geometry against a pinned-source C probe (a sleeping box stays asleep; upstream behaves identically), and added Stage10Tests covering the rebuild and event enablers. Movers and planes (2026-09-01): `World.CastMover` (capsule sweep over all three broad-phase trees with can-encroach semantics, overlap ignoring, and filter rejection, ported from `b2World_CastMover` through the new engine-internal `WorldMoverCastContext`) and `World.CollideMover` (per-shape collision planes forwarded to a `WorldMoverPlaneCallback` through `WorldMoverCollideContext`, ported from `b2World_CollideMover`) close the world-query mover rows; the per-shape collide ported the four upstream `b2CollideMoverAnd*` geometry functions into `Mover.CollideCircle/Capsule/Polygon/Segment` with `Shape.CollideMover` dispatching and rotating the plane normal into world space, and `PhysicsMaths.IsNormalized` ports the upstream unit-vector tolerance. Numbers verified against a pinned-source C probe: the wall-contact sweep fraction 0.326250, the floor-contact sweep fraction 0.701250, and the sunk-mover plane offset 0.05 all match. Three new Stage 10 tests cover the cast (wall, open side, filter rejection, encroachment, floor), the plane registration, and the early-stop callback. Explosions (2026-09-01): `World.Explode` ported from `b2World_Explode` through the engine-internal `ExplosionContext` tree query — per dynamic shape within the blast range it wakes the body and applies the facing-perimeter impulse at the closest point (deep overlaps use the shape centroid), with the linear falloff scale and the local-space projected-perimeter helpers ported from `b2GetShapeProjectedPerimeter`/`b2GetShapeCentroid`; `ExplosionDefinition` carries the upstream defaults. Impulse velocities verified against a pinned-source C probe (inner box (4.327869, 0.393443), falloff box (2.567863, 0.073368), out-of-range zero). The explosion test also caught a real port bug: `World.SetBodyAwake(False)` performed a raw solver-set transfer that left island bookkeeping disagreeing with the body's set (validation threw "Body solver set disagrees with island"); it now ports the upstream island-aware path — split a pending-split island, then `TrySleepIsland` — and the full suite passes without sleep/wake regressions. Two new Stage 10 tests cover the impulse pattern and the implosion direction. Counters and profile (2026-09-01): `World.Counters()` refreshes and returns the world's reusable `WorldCounters` record (body/shape/contact/joint/island counts, static and dynamic tree heights, and the twelve graph-colour constraint row counts, ported from `b2World_GetCounters` with the byte, stack, and task counters excluded as documented) and `World.Profile()` returns the world's reusable `WorldProfile` record measured with the Objo clock at Physics2D's stage granularity (step, broad-phase pairs, collide, solve, sensors — upstream's finer solver stage timings are internal to the port's fused solve pass). The zero-alloc step path only writes into the pre-allocated records. Two new Stage 10 tests pin the counts across an awake-to-sleeping transition (the colour counts empty while the live joint count stays) and the profile fields. Debug rendering (2026-09-02): `World.DrawDebug(renderer, options)` ports `b2World_Draw` through the two upstream paths — the full walk (shapes over solver sets with the shared state-colour ladder, joints, fat AABBs with white body-id labels, body names, mass data, graph-colour contact points, island bounds) and the bounds walk (a `WorldDebugDrawContext` tree query marks visited bodies in a reusable `DebugDrawBodySet` bit set, then a marked-body walk draws names, mass, deduplicated joints, and deduplicated awake contacts). `DebugRenderer` mirrors the nine `b2DefaultDebugDraw` stubs as overridable no-op methods so applications implement only what they can draw; `DebugDrawOptions` carries the upstream flag set (shapes, joints, joint extras, bounds, mass, body names, contacts, graph colours, contact normals/impulses/features, friction impulses, islands) and `DebugColors` ports the used `b2_color*` palette as core `Colour` constants with the graph-colour ladder. `DebugDrawMethods` ports `b2DrawShape`, `b2DrawJoint` with all family decorations (distance limit ticks and spring rest marker, prismatic/wheel axis and limit ticks, revolute hinge circle with angle extras and limit spokes, mouse tether, filter rope, generic dark-sea-green rope) and the two contact-point label dialects; `Shape.CustomColor` converts from the documented 0xRRGGBB integer. Behaviour verified against the pinned upstream source, including the zero-area drawing-bounds validity rule (`b2IsValidAABB` accepts degenerate boxes) and the per-path impulse label conventions recorded as difference 39. The length-scale configuration rows moved to the approved exclusions with the pinned one-metre default documented. Seven new Stage 10 tests cover the shape colour ladder, every joint family's decoration, joint extras with graph colours, bounds mode with names and mass, contact layers with the normals-suppress-impulses else-if chain, island bounds, and argument validation. The desktop adapter `CanvasDebugRenderer` (Demo project, `Import Physics2D`) forwards all nine primitives to the desktop `Graphics` API with an explicit pixels-per-metre scale and no Y-axis flip, and the Demo project builds end-to-end under the desktop profile. While validating the desktop build, two real Objo engine/tooling bugs surfaced and were fixed in the Objo checkout: `VM.BuildArgTypeSignature` overflowed its fixed 128-character stackalloc buffer on nine class-typed arguments (crash fixed with a measured fast path plus heap fallback, regression program `method_long_type_signature.objobasic`, engine suite 4271 passed) and the macOS publish pipeline verified the copied app bundle through a relative path while running codesign inside the bundle's directory (fixed by resolving to full paths, regression test in `MacCodeSigningServiceTests`, Studio codesign tests 14 passed); both Objo commits are local. API audit, examples, reference, and freeze (2026-09-02): the scripted reconciliation is now a rerunnable gate — `tools/audit_api.py` parses all 329 inventory rows and verifies 241 `Class.Member` mappings against the generated dist with zero errors, and it caught two stale rows now fixed (`b2World_SetFrictionCallback`/`b2World_SetRestitutionCallback` map to `World.MaterialMixer`, the implemented `CustomMaterialMixer` surface). The two conditional length-scale rows resolved as approved v1 exclusions (one metre per length unit pinned; screen scale belongs to the render adapter), leaving no unmapped or conditional public symbol. Solver bookkeeping that the compiler requires to be public (set indices, edge keys, linked lists, event buffers, pools, the world's scratch records) is now documented as internal in the source and omitted from the reference; `ApiExamplesTests` (new, six tests) constructs and calls every public overload family — three world construction paths, both step overloads, every shape family with the full definition field set, all eight joint families with definitions and runtime setters, every query overload family in callback and reusable-list forms, movers, the debug-draw option combinations, and the event/lifetime rules. `docs/API.md` is generated from the verified dist declarations by `tools/generate_api_docs.py` with curated categories (no aspirational members by construction; regeneration fails on curated names missing from the dist), and decision 0005 freezes the version 1 API: breaking changes now require a decision record and user approval, internal-infrastructure classes stay unfrozen, and the audit script joins the release audit. Stage 10 is complete: every exit criterion passes — the inventory has no unmapped item, no C leakage remains in public names, the core compiles under the command-line (Tests, Smoke) and desktop (Demo app) profiles, debug drawing works through the fake recording renderer and the compiled desktop adapter, every public declaration is documented with a compile test, and the v1 review is recorded. 331 Physics2D tests plus 19 benchmark tests pass; dist regenerated (sha256 811b4e6b…), clean-room check passes. |
| 11. Interactive demo and teaching examples | Complete | Demo core (2026-09-02): the desktop application now runs the teaching demo on a fixed-timestep accumulator loop — a code-built `DemoWindow` (1200×800, Canvas at 40 pixels per metre) steps the world in whole 1/60 s increments clamped at 0.25 s of catch-up, renders through the Stage 10 `CanvasDebugRenderer`, and draws the HUD (frame rate, `World.Profile()` step time, body/contact/joint/sleeping counts, substeps, pause/CCD/gravity/sleep state, scene help, and scene event logs). All ten planned scenes exist as `DemoScene` subclasses using documented public APIs only: welcome playground (one body per shape family), sleeping pyramid with a delayed wrecking ball, materials rows (friction, restitution, rolling resistance, conveyor tangent speed), sensors and filters (logged sensor events, category/mask pairs, negative group), the seven-family joints gallery, continuous collision with tunnelling bullets and the CCD toggle, cursor ray plus overlap query visuals, chain terrain with a loop, the character mover driven by `World.CastMover`/`World.CollideMover` with plane grounding, and the forty level benchmark pyramid at one substep. Controls: digit scene selection, pause, single step, reset, gravity cycle, CCD and sleeping toggles, F1–F8 debug-draw layer toggles, left-drag mouse-joint body dragging, right/middle-click box and circle spawning, and arrow-key mover input. The `--soak [minutes]` mode cycles every scene automatically and exits, which is the automated soak gate. `docs/DEMO.md` teaches the fixed-step loop, the metres-to-pixels adapter, construction patterns, safe event handling, destruction rules, query reuse, and the per-frame allocation policy. Two Objo desktop-runtime lessons were learned and recorded in AGENTS.md: a Window subclass constructor must call `Super.Constructor()` before the native backing and its events exist (diagnosed with a throwaway probe app after `Opening`/`Opened`/timers silently never fired), and event handlers receive `Me` bound to the event source with `Self` bound to the handler owner. Soak gate (2026-09-02): the demo's soak measures simulated physics time — 108,000 fixed 1/60 s steps (30 simulated minutes, six full ten-scene cycles) counted per executed step so macOS timer throttling of an occluded window cannot stall the gate — and runs flat out with a full catch-up budget per tick, finishing in 7m03s wall with exit code 0. The gate exits non-zero if any runtime error was caught and logged; errors are logged with `RuntimeException.StackFrames` to `/tmp/physics2d-demo-errors.log`, and the tick, paint, and mouse handlers are wrapped in `Try`/`Catch` so swallowed event-loop exceptions stay diagnosable. The first guarded soak run caught a real module bug: an interactive mouse-joint drag destroyed a joint, leaving a Nothing hole in the id-indexed joint array, and `World.DrawDebug`'s joint stage read `SetIndex` on the hole — upstream compacts a dense joint array, so its equivalent loop needs no guard. Fixed by skipping holes before the live check, with regression test `TestDrawDebugSkipsDestroyedJointSlots`; `dist/Physics2D.objobasic` regenerated (sha256 b4b49dfd…). Orientation verification: all ten scenes were screenshotted and pixel-checked after the screen-convention fix (world +Y is screen down; scenes author negative Y above the ground and no longer override `World.Gravity`, which the window's gravity preset owns, so the G key re-tars every scene; only the continuous scene pins zero gravity by design; `--start N` is 1-based per `docs/DEMO.md`). Bodies rest on the ground line at 75% canvas height, the pyramid points up, the joints gallery's seven rigs match their authored coordinates (the wheel hangs at its spring equilibrium, the prismatic slider rests at its down-travel limit), and the character mover capsule rests grounded on the floor. The mover scene was rewritten to the upstream character pattern — `World.CollideMover` planes, `Mover.SolvePlanesTo`, `World.CastMover` clamp, iterate until the delta settles, then `Mover.ClipVector` — replacing a naive vertical sweep that crept through the floor once the capsule touched it. Final gates: 332 Physics2D tests plus 19 benchmark tests pass, and the clean-room distribution check passes. |
| 12. Optimisation and allocation audit | **Complete** (2026-09-04) | Stage 12 coverage, optimisation, and gates are complete. Required scenarios: ten new Stage 12 benchmark scenarios (pyramid-256, sleep-wake-600, tumbler-create-destroy, dense-contacts-240, bullet-ccd-24, world-queries, sensor-field, joint-machine, clear-rebuild, demo-benchmark-pyramid) registered with a `--only` filter, a `--profile` mode, and per-scenario stderr progress; they complete the required matrix with the existing Stage 2/3/4/5/7/9 scenarios. Allocation audit: per-stage allocation instrumentation attributed the three failing zero-allocation gates to real engine sources, then six fixes landed — world-shared contact manifold/cache pools replacing per-store pools (per-store free lists scattered recycled slots and re-allocated on most contact begins), eager manifold collision scratch, world-owned IntegerList split scratch for IslandMethods.SplitIsland, Reserve(rows) on BodySims/BodyStates/ContactSims with pre-sized transfers in TrySleepIsland/WakeSolverSet/MergeSolverSets, amortised fill-doubling for per-colour constraint pools, and scalarised ApplyContinuousHit interpolation — plus an allocation-free world query leaf path (world-owned cast output, transform, shape-side proxies, pair input, and query inputs; callback forms keep allocating outputs so callbacks may retain references). All four Stage 12 gates pass (pyramid, sleep/wake migration at 120 bodies, sensor field with published events, continuous bullets) alongside the existing zero-alloc gates. Optimisation decisions from paired before/after runs on identical scenario definitions: the interrupted contact/solver list-hoisting candidate was REVERTED per the does-not-beat-noise rule (three full-suite configurations — Stage 11 engine, candidate-only, accepted final — produced identical checksums everywhere while identical-code scenarios drifted ±2–10% between runs, showing the earlier +15% pyramid-40 observation was drift; the five touched functions are byte-identical to dbde82c), and the allocation fixes were accepted with decisive measured wins: stage12-world-queries 95.3 s → 1.50 s median in-suite (leaf-path allocations 3.1M → 23,000 after right-sizing the scenario with RebuildStaticTree and a 2,250-query mix), stage12-sleep-wake-600 −29%, stage12-clear-rebuild −15%, stage12-pyramid-256 −8% (allocations 261,313 → 47,130), stage9-joint-chain-60 −52%, and motor/weld/wheel p95 GC spikes eliminated; paired focused runs verified tumbler and joint-machine at parity. Raw results preserved: benchmarks/results/stage12-before-candidate-2026-09-04T20-35-56.json, stage12-candidate-only-2026-09-04T20-52-19.json, and the accepted stage12-final-2026-09-04T21-10-48.json. Determinism held bit-for-bit across all four full-suite runs (every scenario checksum unchanged, including stage7-pyramid-40 161419885332893877). docs/PERFORMANCE.md written with method (drift band, checksum discipline, reproducible commands), results, no-allocation patterns, capacity planning, substep trade-offs, query/event choices, profiling guidance, and honest limitations. Final gates: 336 Physics2D tests plus 19 benchmark tests pass (`objo test --all-projects`), dist regenerated (sha256 f5841102…), `tools/assemble_module.py --check` clean, `tools/audit_api.py` clean (329 rows, 241 references), clean-room `tools/check_distribution.sh` passes. |
| 13. Documentation and release candidate | **Complete** (2026-09-04) | Documentation: README rewritten for the finished port (capabilities, one-screen example, installation, compatibility, docs map, honest performance statement, licence); `docs/GETTING_STARTED.md` (install, world, fixed-step loop, bodies/shapes, render scale, events, destruction, queries, common mistakes) and `docs/CONTRIBUTING.md` (setup, checks, tests, benchmarks, style, docs, artifacts, provenance) added; final consistency review fixed a stale `World.Step` name in PERFORMANCE.md, a stale "(later stages)" diagram note in ARCHITECTURE.md, missing `World.ContactManifoldPool` in API.md (Stage 12 regenerated the dist after the docs), and two non-compiling DEMO.md fragments (bare `Min`, single-quote comment); new `tools/check_doc_samples.py` compiles all 17 `objo` samples in README/GETTING_STARTED/API/DEMO against the dist and every sample now compiles; broken-link check clean. Distribution: version `1.0.0-rc.1` exposed as `Physics2D.VERSION` with decision 0006; `tools/assemble_module.py` mirrors the version plus the minimum-Objo compatibility record and both full MIT notices (Physics2D + Box2D) into the generated header, parsing the constant to prevent drift; `tools/generate_api_docs.py` emits the module-constants section; `Physics2D.Smoke` and the dist consumers unchanged behaviourally. Release-candidate gates: `tools/inspect_distribution.py` (imports, desktop types, markers, debug output, header currency, public-surface documentation — found and fixed an unmarked public `Manifold.mScratch` and a dead `World.mLocalPoint` scratch field; `objo check` now warning-free); clean consumer proof `tools/clean_consumer_check.sh` (dist-only solution builds and runs the README falling-box example verbatim and the Smoke application; discovered and documented that Objo runs the class named `App` as the entry point — a differently named class silently does nothing); single-source installation documented after verifying the Objo checkout has no redistributable source-package format (`SolutionStorageFormat.Package` is directory solution storage, no `.objopackage` exists); `docs/RELEASE_CHECKLIST.md` records the gates and exact SHA-256 hashes. §22 audit: all 13 items executed — audit_api (329 rows/241 refs) plus reverse public-surface check; marker/leakage/silent-catch/skip sweeps clean; static hot-path allocation sweep clean with runtime zero-alloc gates; longest seeded stress and invariant tests green; suite run twice in fresh processes (`objo test` has no order option; 337 Physics2D + 19 benchmark tests, 0 failed/skipped); all 40 benchmark scenario checksums bit-for-bit identical to the accepted Stage 12 record from a clean-checkout Release run (`benchmarks/results/stage13-release-candidate-2026-09-04T22-59-11.json`; the two visible median outliers sit inside the identical-code drift bands of four recorded runs); golden fixtures regenerated from a fresh pinned-commit extraction, byte-identical; dist regenerated three times, one hash; new command-line and desktop consumers built from the documentation alone against the dist only (the desktop one first-try); demo `--soak` gate passed (30 simulated minutes, exit 0, no errors). One §22 item remains for the user: the demo manual interaction pass — the machine was locked during the audit (automated soak ran occluded as designed); Stage 11's screenshot-and-controls orientation pass is the last recorded manual verification. HANDOFF.md deleted per its own instruction; its evidence lives in the ledger, PERFORMANCE.md, and the raw result files. |
