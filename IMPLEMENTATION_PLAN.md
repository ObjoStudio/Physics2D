...
| 9. Joints | In progress | Common joint infrastructure and the distance family implemented in `Shared/Sources`: `JointType` enum (upstream member order), `JointSim` base plus `DistanceJointSim` family record stored as heap objects in `JointSims` wrappers on `SolverSet`/`GraphColor` (heterogeneous-family object-field representation, deviating from decision 0004's parallel arrays — to be justified in `docs/ARCHITECTURE.md`), `Joint` cold record plus common façade with the `DistanceJoint`/`DistanceJointDefinition` family surfaces, `JointSolver` (prepare, warm start, velocity solve with spring, speculative lower/upper limits, clamped motor, and rigid branch, reaction force via cached `World.InvH`), `DistanceJointScratch` reused from `StepContext` so joint solves allocate nothing, `GraphMethods` joint colouring (`ChooseJointColor` static search from colour 0 per upstream b2AssignJointColor), `NewJointSimOfType` factory, `CreateJointInGraph`/`AddJointToGraph`/`CopyJointSim` field clone/`RemoveJointFromGraph`, `IslandMethods` `LinkJoint`/`UnlinkJoint`/`AddJointToIsland`, DFS split joint walk, island validation with joints, `World` joint lifecycle (`CreateDistanceJoint`, `DestroyJointInternal`, `TransferJoint` across solver sets with graph re-colouring, `DestroyContactsBetweenBodies`, upstream placement rules: static+static → static set, disabled → disabled set, either awake → graph, sleeping mix → max sleeping set with `MergeSolverSets`), `SetBodyType`/`Enable`/`Disable`/`DestroyBody` joint handling with static-joint transfers, `ContactMethods.ShouldBodiesCollide` joint walk, `Body.GetJointCount`/`GetJoints`/`HeadJointKey` edge lists, `World.JointCount`/`Validate` joint invariants, and `SolveStep` joint stages with upstream overflow-first ordering; fixed a latent Stage 7 bug where the overflow colour (11) was skipped in the solve-with-bias and relax loops (all prior scenario checksums unchanged), and a pre-existing sizing bug where `mContactStateBits` was cleared with the contact pool's live count instead of its capacity (upstream world.c:552 uses `b2GetIdCapacity`; a 44-step 60-joint chain crashed once contact slots churned past the live count); found and isolated an Objo engine bug — `Super.Constructor()` inside a module fails at runtime with an internal VM type mismatch (top-level classes unaffected; minimal repro held) — so joint record/sim constructors were removed in favour of property defaults plus explicit assignment in `NewJointSimOfType` and the world creation paths, matching the module's existing no-super-constructor convention; `Stage9Tests` (13): rigid constraint, spring restore, limit clamp with motor, motor stretch, collide-connected contact destroy/restore, placement across static/disabled/awake sets, destroy unlinks and wakes, destroy-body cascade, `GetJoints`, slot reuse with stale façades, tuning validation with upstream clamping semantics, 60-link chain stress with contact-state bit regression, and joint-chain determinism; 243 tests pass in `Physics2D.Tests` plus 19 benchmark harness tests (262 total, `objo test --all-projects`), dist regenerated, `objo check` clean; `stage9-joint-chain-60` benchmark (60 bodies/60 joints, median 0.21 s per 4-step iteration, checksum 4283633503840920984) in `benchmarks/results/stage9-joints-2026-09-01T09-28-24.json` with every prior scenario checksum unchanged; remaining: mouse/motor/revolute/prismatic/weld/wheel families each with their common slice, `docs/ARCHITECTURE.md` joint-sim representation section, `docs/PORTING.md` joint mappings and deliberate differences (joint-sim objects, `GetJointType` rename, `b2Body_GetJointCount`/`GetJoints` staging), and an allocation audit of the awake joint path (~5.1k objects per chain iteration, same family as the pyramid baseline) |
